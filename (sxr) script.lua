...
-- Criar todos os botões do painel 1 (ANTI DESYNC TROCADO POR ANTI TORRETA)
...
-- TROCADO: ANTI DESYNC → ANTI TORRETA
...
antiTorretaBtn.MouseButton1Click:Connect(function()
    ...
    if autoSentry then
        antiTorretaBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
        print("✅ ANTI TORRETA ATIVADO")
        StartAutoSentry()
    else
        antiTorretaBtn.BackgroundColor3 = Color3.fromRGB(139, 0, 0)
...
local infJumpHumanoid = nil
local infJumpConnections = {}

-- Setup personagem para INF JUMP
local function setupInfJump(char)
    infJumpCharacter = char
    infJumpHumanoid = char:WaitForChild("Humanoid")
    
    if infJumpHumanoid then
        infJumpHumanoid.UseJumpPower = true
        
        if INF_JUMP_ENABLED then
            infJumpHumanoid.JumpPower = INF_JUMP_POWER
        else
            infJumpHumanoid.JumpPower = INF_JUMP_DEFAULT
        end
    end
end
...
local function enableInfiniteJump()
    INF_JUMP_ENABLED = true
    
    -- Jump Request Connection
    local jumpConnection = UIS.JumpRequest:Connect(function()
        if INF_JUMP_ENABLED and infJumpEnabled and infJumpHumanoid then
            infJumpHumanoid:ChangeState(Enum.HumanoidStateType.Jumping)
        end
    end)
    
    -- Anti Fall Damage
    local antiFallConnection = game:GetService("RunService").Heartbeat:Connect(function()
        if infJumpEnabled and INF_JUMP_ANTI_FALL and infJumpHumanoid then
            if infJumpHumanoid:GetState() == Enum.HumanoidStateType.Freefall then
                infJumpHumanoid:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)
            end
        end
    end)
    ...
    table.insert(infJumpConnections, jumpConnection)
    table.insert(infJumpConnections, antiFallConnection)
    
    print("✅ INF JUMP ATIVADO - Pulo: " .. INF_JUMP_POWER)
end
...
local function disableInfiniteJump()
    ...
    for _, connection in ipairs(infJumpConnections) do
        pcall(function()
            connection:Disconnect()
        end)
    end
    ...
    print("❌ INF JUMP DESATIVADO")
end

-- Configuração do botão INF JUMP
infJumpBtn.MouseButton1Click:Connect(function()
    ...
    if infJumpEnabled then
        ...
        infJumpBtn.BackgroundColor3 = Color3.fromRGB(139, 0, 0)
        disableInfiniteJump()
    end
end)

-- Configuração dos outros botões do painel 2 (mantendo a funcionalidade original)
...
flyV2Btn.MouseButton1Click:Connect(function()
    local ativo = flyV2Btn.BackgroundColor3 == Color3.fromRGB(0, 170, 255)
    if ativo then
...
print("✅ ANTI TORRETA: Auto-destruição de torretas inimigas")
print("✅ INF JUMP: Pulo infinito + anti queda configurado")
