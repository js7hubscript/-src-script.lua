...
local Notification = {
    new = function(self, options)
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = options.Title or "JS7 HUB",
...
-- FUNÇÕES DO HUB (MANTIDAS ORIGINAIS)
...
local function toggleInvisible(state)
    ...
    local function revertClone()
        if not oldRoot or not player.Character or player.Character.Humanoid.Health <= 0 then return false end
...
menuBtnText.Font = Enum.Font.GothamBold
...
Notification:new({
    Title = "JS7 HUB",
    Description = "Painel compacto carregado! Clique no círculo vermelho.",
    Duration = 5
})
